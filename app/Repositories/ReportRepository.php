...
class ReportRepository
{
  ...
  public static function get($inputs)
  {
    // order
      ...
      $dataod = $od->select(DB::raw("sum(orderprice) - sum(coalesce(orderdiscountprice,0)) as total"))->first();
    // pengeluaran
      $ex = Expense::where('expenseactive', '1')
      ->join('users as cr', 'cr.id', '=', 'expensecreatedby' )
      ->whereRaw("expensedate::date between '" . $inputs['startdate'] . "' and '" . $inputs['enddate'] . "'");
      ...
      $dataex = $ex->select(DB::raw("sum(expenseprice) as totalex"))->first();
    return array ($dataod, $dataex);
  }
