		...
		@if(Perm::can(['shift_simpan']))
			<div class="col-12 layout-spacing">
					<div class="alert alert-light-dark" role="alert">
						<strong>Shift Aktif -</strong> {{isset($data->shift->active) ? $data->shift->active->username . " pada " . $data->shift->active->shiftstart : " Tidak ada "}}
						@if(isset($data->shift->active) && $data->shift->data['can_close'])
							<a type="button" href="{{url($data->shift->url)}}" class="btn btn-warning btn-rounded">Tutup</a>
						@elseif($data->shift->data['can_close'])
							<a type="button" href="{{url($data->shift->url)}}" class="btn btn-warning btn-rounded">Buat Shift</a>
						@endif
					</div> 
			</div>
		@endif
